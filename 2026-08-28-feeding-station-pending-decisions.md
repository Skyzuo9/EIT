...
当前允许：Mac 上加载摘要锁定的 P1 GLB，显示 P2 自动分解草案及其覆盖凭据；运行
`demo-simulation` 的导轨、GCR5 comparison pose、4 ml synthetic 搬运和标准
WorkflowTask/Job/Event 合同测试。
...
| ROB-003 | open | 机器人 + CAD 负责人 | 核对 frozen CAD J0 与 URDF `base_link.STL` 的约 44.708 mm 单轴 AABB 差，决定统一 revision 或保留 comparison-only 基座 | 全网格重合声明、部署 base geometry | J1–J6 frozen-snapshot kinematic-frame registration；demo comparison overlay |
...
1. 公开 Material Graph 为 `unilab-z-up/mm/degree`，`pascal-y-up` 只允许作为 renderer 内部字段；
2. 激活请求 exact 绑定 station geometry、P2 layout、GCR5 topology 与 CAD comparison pose 摘要；
3. 请求固定为 `mode=demo-simulation`、`hardware_execution=false`、`publication_eligible=false`；
4. WorkflowTask 的成功只解释为进程内 visualization actions 成功，不得写成现场设备成功；
5. GCR5 root/joint pose 固定标记 `cad-comparison-only`、`not_calibrated=true`、`not_a_deploy_base_pose=true`；
6. 导轨位姿、瓶的 attached/destination 和搬运动作为 synthetic，不进入部署事实。
